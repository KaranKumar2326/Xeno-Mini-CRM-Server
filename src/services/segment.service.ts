...
import { RuleGroup, Rule, Operator } from '../types';

export class SegmentService {
  private static operatorMap: Record<string, string> = {
    '>': '$gt', '<': '$lt',
    '>=': '$gte', '<=': '$lte',
    '==': '$eq', '!=': '$ne'
  ...
  static async evaluateRules(inputRules: any) {
    try {
  ...
  private static normalizeAndValidateInput(input: any): RuleGroup {
  if (
    input &&
    typeof input === 'object' &&
    'condition' in input &&
  ...
  private static buildMongoQuery(rules: RuleGroup): any {
    const mapper = (r: Rule | RuleGroup) => {
  ...
  private static buildCondition(rule: Rule): any {
  ...
  private static normalizeOperator(op: string): Operator {
    const operator = op.replace('\u003e', '>').replace('\u003c', '<') as Operator;
  ...
  private static parseValue(rule: Rule): number | Date {
    try {
  ...
  private static validateField(field: string): void {
  ...
  private static validateValueType(rule: Rule): void {
