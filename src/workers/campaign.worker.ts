...
const processBatch = async (updates: DeliveryUpdate[]) => {
  await connectDB(); // Ensure DB connection

  const campaignUpdates = new Map<string, { sent: number; failed: number }>();
  const logEntries: { campaignId: string; customerId: string; status: 'sent' | 'failed'; error?: string; timestamp: Date; message?: string }[] = [];
  ...
  await Promise.all([
    Campaign.bulkWrite(campaignBulkOps),
    CommunicationLog.bulkWrite(logBulkOps)
  ]);
};
